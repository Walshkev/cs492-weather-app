...
class _LocationTabWidgetState extends State<LocationTabWidget> {
  ...
  Future<void> _loadSavedLocations() async {
    ...
    final file = File('${directory.path}/saved_locations.json');
    if (await file.exists()) {
      final contents = await file.readAsString();
      final List<dynamic> jsonData = json.decode(contents);
      setState(() {
        _savedLocations.clear();
        _savedLocations.addAll(jsonData.map((data) => location.Location.fromJson(data)).toList());
      });
    }
  }
  ...
  Future<void> _saveLocations() async {
    final directory = await getApplicationDocumentsDirectory();
    final file = File('${directory.path}/saved_locations.json');
    final jsonData = json.encode(_savedLocations.map((loc) => loc.toJson()).toList());
    ...
    return Column(
      children: [
        ...
        SavedLocationsWidget(
            locations: _savedLocations, setLocation: widget._setLocation)
      ],
    );
  }
}
...
class SavedLocationsWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: _locations
          .map(
              (loc) => SavedLocationWidget(loc: loc, setLocation: _setLocation))
          .toList(),
    );
  }
}
